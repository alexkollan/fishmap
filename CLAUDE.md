...
### Map factor layers (`/map`'s Layer Drawer) — a backend-precomputed weather-map overlay, separate from the coastline

The coastline line always shows **Overall**, full stop — `useCoastlineScoring.ts`'s `paint()` no longer branches on the selected score layer. Picking a factor (Wind/Waves/Pressure/Sea temp/Water clarity/Current/Time of day) instead shows a full-viewport translucent color overlay across the whole visible map (sea *and* land — no masking, matching how consumer weather-map apps like Windy render), sourced from a **separate backend-precomputed grid**, not live client-side scoring:

- `apps/api/src/lib/areaGrid.ts` defines a 0.1° grid (~11km, ~7,844 cells) over the Greek bbox — finer than the 0.25° point/coastline grid, but stored in the **same** `weather_cache` table (identical key shape), so no schema change was needed.
- `apps/api/src/jobs/areaWeatherRefresh.ts`, a new cron (`0 */12 * * *` + once at startup, registered in `server.ts`), fetches the whole grid from Open-Meteo and lands it there. **This is paced deliberately (2s between batches) and retries 429/503 with backoff (`open-meteo.ts`'s `fetchOpenMeteoBatch`, respects `Retry-After`)** — firing all ~79 batches back to back with no pacing was tested directly and blew straight through Open-Meteo's per-minute rate limit almost immediately, and the existing `cachedBatch` stale-while-revalidate fallback silently swallows a failed batch as "no data" with no error surfaced anywhere. Don't remove the pacing/retry to "speed it up" without re-testing against the real rate limit.
- `GET /api/scores/area?mode=&hourIndex=` (`apps/api/src/routes/scores.ts`) only ever reads that cache — never calls upstream itself, which is what keeps it fast enough to hit on every hour-scrub. It deliberately does **not** require one specific cell (e.g. the geographic center) to have data before responding — the cache fills in incrementally (especially right after a fresh deploy, or with the pacing above, over its first ~3-4 minutes), so it picks whichever cell has data first for the shared sun/moon reference calc and serves whatever's cached, with `null` elsewhere, rather than an all-or-nothing 503.
- Individual factor scores **don't depend on the weight profile** — only *Overall* does (weight only matters when combining factors into one number). That's what makes precomputing them on a 12h cadence safe without breaking the live admin weight editor described below: Overall keeps scoring live, per-segment, against `useWeightProfiles()`, completely unchanged.
- Frontend: `useAreaScoreGrid.ts` fetches the grid and hands it to `areaHeatmap.worker.ts`, which paints one pixel per grid cell onto an `OffscreenCanvas` (color via `scoreColor.ts`, a JS port of the coastline's ramp — keep the two in sync by hand if either changes) and transfers back an `ImageBitmap` — **no manual interpolation**; the smoothing comes from MapLibre's own `raster-resampling: "linear"` stretching that coarse image across the viewport via an `image` source. That source/layer is inserted with `beforeId: COASTLINE_LINE_LAYER_ID` so it renders below the coastline line and below the overlay tile layers (bathymetry/posidonia/seamarks), which are unaffected and still always end up on top.
