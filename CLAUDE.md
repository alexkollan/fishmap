...
2. Then a backend-precomputed full-viewport factor overlay (0.1° grid over the Greek bbox, ~4,500 sea cells after excluding land, a 12h refresh cron) replaced it — and even after three separate optimizations (TTL reuse, land exclusion, trimming requested variables under Open-Meteo's 10-variable billing threshold), a real instrumented test showed a genuinely cold sweep took **115 minutes** and would recur on every single 12h cron tick indefinitely (TTLs were all far shorter than the cron interval), not just once after a deploy. That was the point it got cut entirely rather than optimized further.

**If map-wide scoring is ever wanted again**, know going in: Open-Meteo's free tier bills locations and variables into the same budget (confirmed empirically, not just from their docs — a burst of ~6 successful 100-location requests reliably tripped their per-minute cap, consistent with something like "locations × requests" being the real unit, not raw request count), so *any* design that scores more than a handful of points needs either a paid Open-Meteo tier (their pricing page: paid tiers remove the minute/hour/day caps, monthly-budget only) or a fundamentally different data source — tuning batch size/pacing/variable count on the free tier was tried thoroughly and tops out nowhere near "instant."

Wind scoring on the map (via `SpotSheet`) is therefore always speed-only, no coastline aspect to be relative to — same as every single-point page; this was already true before the final cut, unaffected by it.
