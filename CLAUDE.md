...
### Map (`apps/web/src/map/`, `/map` route) — deliberately minimal, no score layer at all (2026-08-25)

The map has no *scoring* on it in any form — no coastline line, no factor/weather score overlay, no time scrubber. It's just the base map, a pin at the active location (`MapCanvas.tsx`'s marker effect, tracks `useActiveLocation()`), and tap-to-inspect: tapping anywhere calls `MapPage.tsx`'s `handleTap`, which sets the active location to the tapped point and opens `SpotSheet` — the same single-point `useConditions()` lookup every other page in the app uses (`GET /api/weather?lat=&lon=`, snapped to the 0.25° point grid, 60min/3h TTL-cached). This is the end state of two rounds of cuts, both in `PROGRESS.md` (2026-08-24, 2026-08-25) — worth reading if you're tempted to rebuild any kind of map-wide scoring:
